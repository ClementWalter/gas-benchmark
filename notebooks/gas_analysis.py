...
#' the `uint16` limit (<% 2 ** 16 %> items). We also compare the usage of the native `bytes` array type with the more complex `uint[]` one.
...
#%%
#+write_batch_indexes, echo=False, caption='Gas usage of adding a batch of indexes to the contract storage'
df = pd.read_json("data/writeBatchIndexes.json")
df.plot(logy=True)
plt.legend(bbox_to_anchor=(1.1, 1.05))
plt.show()

#' For a batch size of 128, one has:
#+ last_batch_indexes, echo=False
...
#%%
#+write_batch_indexes_and_addresses, echo=False, caption='Gas usage of adding a batch of indexes and addresses to the contract storage'
df = pd.read_json("data/writeBatchIndexesAndAddresses.json")
df.plot(logy=True)
plt.legend(bbox_to_anchor=(1.1, 1.05))
plt.show()
...
#' For a batch size of 128, one has:
#%%
#+ last_batch_indexes_and_addresses, echo=False
df.iloc[-1].sort_values()
...
#' being <% round(df.iloc[-1].loopTokenIds256ToAddressMapping / df.iloc[-1].concatBytes2ForAddress) %> times more efficient than the
#' standard
#' OZ ones.
#'
...
#'
#%%
#+check_write_indexes_addresses, echo=False, caption='Gas usage of adding a batch of 128 indexes with existence check'
df = pd.read_json("data/checkAndWriteBatchIndexesAndAddresses.json")
df.plot(logy=True)
plt.legend(bbox_to_anchor=(1.1, 1.05))
plt.show()
...
#+ events_single_token, echo=False
pd.Series(json.load(open("data/events.json")))
...
#+mint_batch, echo=False, caption='Gas usage of minting a batch tokens for different implementations'
df = pd.read_json("data/mintBatch.json")
df.plot(logy=True)
plt.legend(bbox_to_anchor=(1.1, 1.05))
plt.show()
